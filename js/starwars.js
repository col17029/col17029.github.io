// helper function to fetch the data from an external source and return it in 
// JSON format
function getJSON(url) {
  ...
  // model code...it is a bit redundant in this case...we could just call 
  // getJSON directly...but if our model became more complex this sets us up 
  // to accomodate that.
  ...
  function renderShipList(ships, shipListElement) {
    // I decided to use a table to display my list of ships. The shipList Element is 
    // that table and it has 2 children: thead and tbody...we need to put our 
    // ships into tbody...so I reference the second child.
    ...
    ships.forEach(function (ship) {
      ...
      listItem.addEventListener("click", function (event) {
        ...
        // and the ship details function should be called...passing the 
  ...
  function showShips(url = "https://swapi.dev/api/starships/") {
    getShips(url).then(function (data) {
      ...
      if (data.next) {
        ...
        // In this case that is a good thing however. Because we are not re-creating 
        // the buttons each time we load a new batch of data we could end up with 
        // several listeners attached to each button by the last page. 
        // We won't have that issue here.
        next.ontouchend = () => {
          // notice to show the next page we just re-call the showShips function 
          // with a new URL
          showShips(data.next);
        };
      }
